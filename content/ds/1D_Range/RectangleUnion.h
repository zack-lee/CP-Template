...
int area(vector<pair<pi,pi>> v) { // area of union of rectangles
	LazySegTree<int> L;
	...
	trav(t,v) {
		t.s.f = lower_bound(all(y),t.s.f)-begin(y);
		t.s.s = lower_bound(all(y),t.s.s)-begin(y)-1;
