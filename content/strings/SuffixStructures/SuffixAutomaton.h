...
struct SuffixAutomaton {
	...
	vi distinct;
	int getDistinct(int x) {
		if (distinct[x]) return distinct[x];
		distinct[x] = 1; 
		trav(y,st[x].next) distinct[x] += getDistinct(y.s);
		return distinct[x];
	}
	int numDistinct() { // # of distinct substrings, including empty
		distinct.resize(sz(st));
		return getDistinct(0);
	}
	int numDistinct2() { // another way to get # of distinct substrings
		int ans = 1;
